                            ...
                            <div class="card-body">
                                <div class="w-full py-2">
                                    <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5">
                                        ...
                                        <input class="input" id="name" name="name" placeholder="Enter Ad Title" type="text" value="{{$ad->title}}" required />
                                    </div>
                                </div>
                                <div class="w-full py-2">
                                    <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5" style="flex-direction: column;align-items: stretch;">
                                        ...
                                        <textarea class="textarea" id="description" name="description" placeholder="Text" rows="4" required>{!!$ad->description!!}</textarea>
                                    </div>
                                </div>
                                <div class="input-group w-full py-2">
                                    ...
                                    <input class="input" id="price" name="price" placeholder="Enter Price" type="text" value="{{$ad->price}}" />
                                    <div class="dropdown" data-dropdown="true" data-dropdown-trigger="click">
                                        <button class="dropdown-toggle btn btn-primary">
                                            <span id="selected-currency">{{App\Models\Currency::CURRENCY[$ad->currency_id]}}</span>
                                ...
                                <div class="w-full py-2" style="display: flex;gap: 10px;">
                                    <select class="select" name="user" id="user">
                                        ...
                                        @foreach($users as $usr)
                                            <option value="{{ $usr->id }}" @if($ad->user_id == $usr->id) selected @endif>{{ $usr->first_name }} {{ $usr->last_name }} - {{ $usr->mobile }}</option>
                                        @endforeach
                                    </select>
                    ...
                    <div class="col-md-6 mb-4">
                        <div class="card" style="height: 560px;">
                            ...
                            <div class="card-body" style="height: 387px;overflow-y: auto;">
                                @php
                                    $category = App\Models\Category::find($ad->category_id);
                                    $ancestors = $category ? $category->ancestors()->pluck('name')->toArray() : [];
                                    // نضيف اسم التصنيف الحالي في النهاية
                                    if ($category) {
                                        $ancestors[] = $category->name;
                                    }
                                    $fullPath = implode(' -> ', $ancestors);
                                @endphp
                                <p>Current Category: {{ $fullPath }}</p>
                                <hr>
                                <div id="catTree"></div>
                                <input type="text" id="catId" style="display:none;" value="{{$ad->category_id}}">
                            </div>
                        </div>
                    </div>
                </div>
...
//       function setDirectionByContent(content) {
//         const arabicRegex = /[\u0600-\u06FF]/;

//         if (content && arabicRegex.test(content)) {
...
//       // عند تشغيل المحرر أول مرة (init)
//       editor.on('init', function () {
//         let initialContent = editor.getContent({ format: 'text' }).trim();
//         setDirectionByContent(initialContent);
//       });

//       // عند الكتابة أو التغيير
//       editor.on('keyup change', function () {
//         let content = editor.getContent({ format: 'text' }).trim();
//         setDirectionByContent(content);
//       });
//     }
//   });
</script>
<script>
    document.addEventListener('DOMContentLoaded', function() {
            ...
            if (cityId) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        const selectedAreaId = {{ $ad->area_id ?? 'null' }};

                        // Populate the area dropdown with new options
                        response.data.forEach(function(area) {
                            let isSelected = (area.id == selectedAreaId) ? 'selected' : '';
        ...
        $("#save").click(function(e) {
            ...
            let adUpdateId = @json($ad->id);
            $.ajax({
                url: '/admin/ads/'+adUpdateId+'/update',
                type: 'POST',
                data: {
                  "title": title,
                ...
                success: function(response) {
                    if(response.success && response.data && response.data.ad) {
                        ...
                        if (myDropzone.getQueuedFiles().length > 0) {
                            // إذا في صور جديدة نرفعها
                            myDropzone.processQueue();
                        } else {
                            // إذا ما في صور، نعرض النجاح مباشرة
                            Swal.fire({
                ...
                error: function(xhr, status, error) {
                    ...
                    if (xhr.responseJSON) {
                        ...
                        // إذا بدك تعرض أول خطأ من errors[]
                        if (xhr.responseJSON.errors) {
                            const firstKey = Object.keys(xhr.responseJSON.errors)[0];
                            if (xhr.responseJSON.errors[firstKey] && xhr.responseJSON.errors[firstKey][0]) {
    ...
    $(document).ready(function() {
        ...
        let adAttributes = @json(
            $ad->adAttributes->mapWithKeys(function ($attr) {
                if ($attr->relationLoaded('options') && $attr->options->isNotEmpty()) {
                    // multiselect: array of selected IDs
                    return [$attr->attribute_id => $attr->options->pluck('ad_attribute_option_id')->map(fn($id) => (string) $id)->values()];
                }

                return [$attr->attribute_id => (string) $attr->value];
            })
        );
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    response.data.forEach(item => {
                        if (item.id > 0) {
                            let inputElement = '';
                            let value = adAttributes[item.id] || '';

                            switch (item.type) {
                                case 'text':
                                    inputElement = `<input class="input" type="text" data-id="${item.id}" name="${item.title}" placeholder="${item.title}" value="${value}" ${item.required == '1' ? 'required' : ''}>`;
                                    break;

                                case 'date':
                                    inputElement = `<input class="input" type="date" data-id="${item.id}" name="${item.title}" placeholder="${item.title}" value="${value}" ${item.required == '1' ? 'required' : ''}>`;
                                    break;

                                case 'numeric':
                                    inputElement = `<input class="input" type="number" data-id="${item.id}" name="${item.title}" placeholder="${item.title}" value="${value}" ${item.required == '1' ? 'required' : ''}>`;
                                    break;

                                case 'select':
                                case 'radio':
                                case 'dropdown':
                                    inputElement = `<select class="select" data-id="${item.id}" name="${item.title}" ${item.required == '1' ? 'required' : ''}>
                                                    <option value="">Select ${item.title}</option>`;
                                    item.options.forEach(option => {
                                        inputElement += `<option value="${option.id}" ${option.id == value ? 'selected' : ''}>${option.value}</option>`;
                                    });
                                    inputElement += `</select>`;
                                    break;

                                case 'multiselect':
                                    let selectedValues = Array.isArray(adAttributes[item.id]) ? adAttributes[item.id] : [];

                                    inputElement = `<select class="select" data-id="${item.id}" name="${item.title}[]" multiple ${item.required == '1' ? 'required' : ''}>
                                                    <option value="">Select ${item.title}</option>`;

                                    item.options.forEach(option => {
                                        inputElement += `<option value="${option.id}" ${selectedValues.includes(String(option.id)) ? 'selected' : ''}>${option.value}</option>`;
                                    });
                                ...
                                case 'boolean':
                                    ...
                                    inputElement = `<input type="checkbox" data-id="${item.id}" name="${item.title}" value="1" ${checked} ${item.required == '1' ? 'required' : ''}> ${item.title}`;
                                    break;
...
<script>
    function initMap() {
        let lat = parseFloat(@json($ad->lat)) || 0;
        let lng = parseFloat(@json($ad->lng)) || 0;
...
<script>
    ...
    var myDropzone = new Dropzone("#my-dropzone", {
        ...
        init: function() {
            var dropzone = this;
            // Existing images
            fetch('/admin/ads/get-ad-images/{{ $ad->id }}')
                ...
                .then(images => {
                    images.forEach(function (image) {
                        ...
                        // نربط ID الصورة حتى نعرف نعمل حذف
            ...
            this.on("success", function(file, response) {
                Swal.fire({
                        text: "Add has been successfully updated!",
                        icon: "success",
                        buttonsStyling: false,
                        confirmButtonText: "Ok, got it!",
                        customClass: {
            ...
            this.on("removedfile", function (file) {
                if (file.serverId) {
                    fetch(`/admin/ads/images/${file.serverId}`, {
                        method: 'DELETE',
                        headers: {
